...
| `full` | Stage 2 ran and something cleared the threshold: an exact entity-id match (always counted as a strong hit), a summary whose cosine similarity to the embedded query is ≥ `0.40` (`vectorSimilarityThreshold`), an episode whose own similarity clears ≥ `0.55` (`episodeVectorSimilarityThreshold`), or an entity whose own embedding clears its cutoff (`entityVectorSimilarityThreshold`, `0.50`). All three are calibrated separately, not one shared number — measurement showed episodes and entities both need a *higher* bar than summaries: a summary is grounding-checked prose, but a raw episode never goes through grounding, and a generic "should I use Postgres or Redis in general?" question measured *higher* similarity against a real episode (0.51) than a genuine broad paraphrase of that same episode (0.45) did — reusing the summary's 0.40 would have let that generic question through as if it were the user's own memory. All three constants are in `internal/store/retrieve.go`, empirically tuned to `text-embedding-3-small`, see each one's own comment for the full measured data. The entity-embedding path (`schema/0012_entity_embeddings.sql`) exists specifically for a query that's a genuine paraphrase of a known entity rather than a literal restatement of its name — previously that case missed entirely, since entities were reachable only by the stage-1 substring check. |
